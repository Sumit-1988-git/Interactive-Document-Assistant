...
if st.button("Get Answer"):
    if user_query:
        ...
        st.markdown(f'<div style='color: yellow; font-weight: bold;' class="chat-box"><div class="chat-bubble user">{user_query}</div></div>', unsafe_allow_html=True)
        response = retrieval_chain.invoke({"input": user_query})
        st.write(response["answer"])
        # Display bot's response as a chat bubble
        st.markdown(f'<div style='color: yellow; font-weight: bold;' class="chat-box"><div class="chat-bubble bot">{response}</div></div>', unsafe_allow_html=True)
    else:
